...
$attachDir='upload/doc';//上传文件保存路径，结尾不要带/这里不能改，影响其他操作
$dirType=2;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录 4:按文件标志记录  建议使用按天存 
$wjbz=$_GET['wjbz']; //只$dirType=4的是时候起作用。
$maxAttachSize=20971520;//最大上传大小，默认是20M
$upExt='txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid,mp4,doc,xls';//上传扩展名
...
$err = "";
$msg = "''";
$tempPath=$attachDir.'/'.date("YmdHis").mt_rand(100,999).'.tmp';
$localName='';
	...
	if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
	{
		...
		else
		{
			switch($dirType)
			{
				...
				case 2: $attachSubDir = 'm_'.date('ym'); break;
				case 3: $attachSubDir = 'ext_'.$extension; break;
				case 4: $attachSubDir = $wjbz; break;
			}
